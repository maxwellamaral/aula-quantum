...
print(f"Probabilidade de ler 0: {prob_0 * 100:.1f}%")
print(f"Probabilidade de ler 1: {prob_1 * 100:.1f}%")
```
...
```{python} {pyodide-python}
soma = prob_0 + prob_1
print(f"Soma das probabilidades: {soma:.4f} (100% garantido)")
assert np.isclose(soma, 1.0)
```
...
```{python} {pyodide-python}
# Instalação do motor quântico Cirq no navegador
import micropip
await micropip.install("cirq-core==1.6.1")
import cirq

# Criando 1 qubit e aplicando a porta Hadamard (H) para gerar a superposição
q0 = cirq.LineQubit(0)
circuito = cirq.Circuit(
    cirq.H(q0),
    cirq.measure(q0, key="resultado")
)

print("Circuito gerado:")
print(circuito)

# Simulando 1000 medições (shots)
simulador = cirq.Simulator()
resultado = simulador.run(circuito, repetitions=1000)
contagens = resultado.histogram(key="resultado")

print("\nResultados após 1000 medições:")
print(f"Estado 0: {contagens.get(0, 0)} vezes ({contagens.get(0, 0)/10:.1f}%)")
print(f"Estado 1: {contagens.get(1, 0)} vezes ({contagens.get(1, 0)/10:.1f}%)")
```
