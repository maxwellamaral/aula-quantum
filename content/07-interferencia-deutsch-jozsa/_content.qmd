...
- ✅ **Superposição** - de [01-intro.ipynb](/notebooks/01-intro.ipynb)
- ✅ **Emaranhamento** - de [02-teletransport.ipynb](/notebooks/02-teletransport.ipynb)
- ✅ **Porta Hadamard** - de [04-Hadamard-experiment.ipynb](/notebooks/04-Hadamard-experiment.ipynb)
...
Lembra da porta CNOT em [01-intro.ipynb](/notebooks/01-intro.ipynb)?
...
Segue o código completo para demonstrar o Phase Kickback usando Cirq. Logo após, faremos uma análise detalhada de cada etapa.
...
# Demonstração do Phase Kickback em Cirq
q_controle = cirq.LineQubit(0)
q_alvo = cirq.LineQubit(1)

qc = cirq.Circuit(
    # 1. PREPARAÇÃO: Controle em |+⟩ e Alvo em |-⟩
    cirq.H(q_controle),
    cirq.X(q_alvo),
    cirq.H(q_alvo),
    
    # 2. O KICKBACK: CNOT faz a fase (-1) chutar de volta para o controle!
    cirq.CNOT(q_controle, q_alvo),
    
    # 3. VERIFICAÇÃO: Hadamard converte |-⟩ em |1⟩ com certeza absoluta
    cirq.H(q_controle),
    cirq.measure(q_controle, key="controle")
)

print("Circuito de Phase Kickback:")
print(qc)

sim = cirq.Simulator()
resultado = sim.run(qc, repetitions=1000)
contagens = resultado.histogram(key="controle")
print("\nResultado da Medição do Controle (1000 repetições):", dict(contagens))
print("O controle mediu 1 em 100% das vezes, provando o Phase Kickback!")
```
...
Lembra disso acima? Se não, volte para a seção de produto tensorial em [00-math.ipynb](/notebooks/00-math.ipynb).
...
```{python} {pyodide-python}
# ============================================
# DEFINIÇÃO DOS ORÁCULOS NO CIRQ
# ============================================

def criar_oraculo_balanceado(qubits_in, qubit_target):
    """
    Retorna as operações para uma função BALANCEADA.
    Implementação: CNOTs dos qubits de entrada para o qubit alvo.
    """
    return [cirq.CNOT(q, qubit_target) for q in qubits_in]

def criar_oraculo_constante(qubits_in, qubit_target, valor=0):
    """
    Retorna as operações para uma função CONSTANTE (sempre 0 ou sempre 1).
    """
    if valor == 1:
        return [cirq.X(qubit_target)]
    return []  # Identidade para constante 0
```
...
- ✅ **Superposição** - você aprendeu em [01-intro.ipynb](/notebooks/01-intro.ipynb)
- ✅ **Emaranhamento** - você aprendeu em [02-teletransport.ipynb](/notebooks/02-teletransport.ipynb)  
- ✅ **Interferência** - você aprendeu em [04-Hadamard-experiment.ipynb](/notebooks/04-Hadamard-experiment.ipynb)
...
- **Cirq Textbook:** [Deutsch-Jozsa Algorithm](https://cirq.org/textbook/ch-algorithms/deutsch-jozsa.html)
